...
from consulta_expediente.django_assertions import assert_contains, assert_not_contains
...
def test_login_redirect(resp_post):
    ...
    assert resp_post.url == reverse('base:consulta')
...
def test_botao_entrar_disponivel(resp_home):
    assert_contains(resp_home, 'Acceder')
...
def test_botao_entrar_indisponivel(resp_home_com_usuario_logado):
    assert_not_contains(resp_home_com_usuario_logado, 'Acceder')
